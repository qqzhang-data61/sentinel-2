...
for f in $(ls $SRC_DIR/*.img); do n=${f##*/} ; gdalwarp -t_srs wgs84.prj -r cubic -srcnodata 0 -dstnodata 0 -of GTiff -overwrite $f $DST_DIR/$n-$TIME_SUFFIX.wgs84.tif ;  done
    
for i in $(ls $DST_DIR/*-$TIME_SUFFIX.wgs84.tif); do gdalbuildvrt -vrtnodata "0" -r cubic $i-vrt.vrt $i ; done

gdalbuildvrt -vrtnodata "0" -r cubic $DST_DIR/$DST_NAME.vrt $PATCH_TIF $(ls $DST_DIR/*-vrt.vrt)
